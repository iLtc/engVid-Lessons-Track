...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference('Config.count') do
      post configs_url, params: { config: { name: @config.name, title: @config.title, value: @config.value } }
    end
  ...
  test "should update config" do
    patch config_url(@config), params: { config: { name: @config.name, title: @config.title, value: @config.value } }
